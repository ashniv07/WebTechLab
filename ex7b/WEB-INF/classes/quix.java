...
public class quix extends HttpServlet {
    private int counter = 0;

    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            Class.forName("com.mysql.jdbc.Driver");
            Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3307/quiz", "root", "");
            for (int i = 1; i <= 5; i++) {
                String user_ans = request.getParameter("answer" + i);
                PreparedStatement stmt = conn.prepareStatement("SELECT * FROM quiz WHERE sno = ?");
                stmt.setInt(1, i);
